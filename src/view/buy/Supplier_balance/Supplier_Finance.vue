 ...
 * @Last Modified time: 2019-11-26 09:05:19
 */
<!--
 * @Description: 客户余额表
 * @Author: wb
...
export default {
    ...
    data(){
      return {
        ...
        columns4: [
                    ...
                    // },
                    //供应商财务票额表；只查询类型 4和7 的；4对应是 应收票款；7对应 是已收票款；未收票款 = 4-（7+8）
                     {
                        title: '供应商未收票额',
                        key: 'key1',
                        align: 'center',
                    ...
                    },
                    {
                        ...
                        render:(h,params) =>{
                          return h('div',{
                          },this.$global.isMoneyShow(params.row.inMoney4))
                        }
    ...
    methods:{
      ...
      getDian(item){
        ...
        var jsonStr =item? eval('('+item.value+')'):[];
        console.log(item)
        ...
        //  param.userId      = this.adminSearchId  //管理员查询
          ...
          param.bankJgId = this.$global.OMRBankJgId(this.formItem.bankJgId);
           findOrgGroup(param).then(res=>{
            ...
            this.data1.map(item=>{
              //供应商未收票额
                          let a = this.$global.accAdd(item.inMoney7,item.inMoney8)
                          let b = this.$global.accMinus(item.inMoney4,a)
                item.key1 = b
                //已收票额
                          let c = this.$global.accAdd(item.inMoney7,item.inMoney8)
                item.key2 = c;
            })
          }  
         })
