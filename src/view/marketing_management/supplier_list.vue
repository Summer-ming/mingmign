...
<template>
  <div>
    <!-- 新增供应商 -->
    <Modal 
    v-model="addModal" fullscreen  title="新增供应商"  z-index="1" :z-index="1"    
   ...
   <Modal 
    ...
    v-model="modal_review" fullscreen  title="审核公司信息"   
     ...
     <div slot="footer">
      ...
      <div >
        <Card :bordered="false">
          <Button type="info" icon="md-add" ghost @click="addModal = true">新增供应商</Button>
  ...
  methods:{
    ...
    searchEvents(){
     ...
     handleSuccess (res, file) {
                this.isShowCp= true;
                this.$Message.success('上传成功,合同图片')
