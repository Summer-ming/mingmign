...
<template>
  <div>
    ...
    v-model="modal"  :title="modalTitle"  z-index="1" :z-index="1"    @on-ok="modal_ok" @on-cancel="modal_cancel"
     ok-text="提交" width="1000" :styles="{top:'10px'}" >
      <Form :rules="ruleValidate" :label-width="120" inline :model="addForm">
         <FormItem label="销售退货" prop="name">
           <Input style="width:300px" v-model="addForm.name" placeholder="请输入销售退货"/>
         </FormItem>
    ...
    <Modal v-model="modal_delete"
      ...
      <div >
        <Card :bordered="false">
          <Button type="info" icon="md-add" ghost @click="addNew()">新增销售退货</Button>
...
export default {
  ...
  data(){
    return {
      ...
      ruleValidate:{
        name:[{
          required:true,message:'请输入销售退货',trigger:'blue'
        }],
        payMoney:[{
  ...
  methods:{
    ...
    modal_ok(){ //提交保存
      ...
      let param = {};
        ...
        param.payCause            = this.addForm.name //销售退货
