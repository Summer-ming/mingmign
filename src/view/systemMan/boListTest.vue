...
<template>
     ...
     <Form :model="formItem" inline :label-width="100">
        <Row>
          ...
          <Col span="24">
             <Button type='primary' icon="ios-search" @click='serachBtn()'>查询</Button>
            ...
            <Button type="primary"  icon="ios-download-outline" @click="exportData()" style='margin-right:10px'>导出excel</Button>
             <Button style="margin-left:10px"   type='primary'  @click="getSelectEvent1()">批量修改对账日期</Button>
              <!-- 合计 -->
              <span style="color:#4b98dc;padding-left:20px;float:right;">合同重量<span   style="color:#4b98dc;">{{this.$global.accPrecision(TWeight,3)}}</span> 吨&nbsp;&nbsp; </span>
              <span style="color:#4b98dc;float:right;">合同金额<span  style="color:#4b98dc;">{{this.$global.isMoneyShow(TMoney)}}</span> 元&nbsp;&nbsp;</span>
              <br>
              <span style="color:red;padding-left:20px;float:right;">结算总重量<span   style="color:red;">{{this.$global.accPrecision(TjiesuanWeight,3)}}</span> 吨&nbsp;&nbsp;</span>
              <span style="color:red;float:right;">结算总金额<span   style="color:red;">{{this.$global.isMoneyShow(TjiesuanMoney)}}</span> 元&nbsp;&nbsp;</span>
          </Col>
        </Row>
        ...
        <vxe-table height="400"
         class="mytable-style"  :select-config="{labelField: 'name', checkMethod}" size="mini" ref="xTable1" :data.sync="data1"  >
                ...
                <vxe-table-column field="orderNo" title="销售单号" width="100" >
                ...
                <!-- //下面是显示 -->
                <vxe-table-column width="120" field="createTime" title="下单时间" >
                  <template v-slot:header>
                      <span>下单时间</span>
                        <DatePicker :transfer="true"  @on-change="changeCreateTieme" format="yyyy-MM-dd" v-model="formItem.dateTime" type="daterange" :options="options" placement="bottom-end" placeholder="选择日期" style="width: 100px">
                        </DatePicker>
                  </template>
                </vxe-table-column>
                <vxe-table-column width="120" :formatter="orderStatus"  field="status" title="订单状态" >
                    <template v-slot:header>
                      <span>订单状态</span>
                      <Select  :transfer="true" v-model='formItem.orderStatus' style="width:120px" placeholder="请选择订单状态" >
                        <Option  v-for='(item,index) in orderStatusList' :value="item.value" :key='index'>{{item.label}}</Option>
                      </Select>
                    </template>
                </vxe-table-column>

                <vxe-table-column width="120" :formatter="orgShopName" field="shopOrgName" title="供应商名称" ></vxe-table-column>
                <vxe-table-column width="120" field="customerOrgName" title="客户" ></vxe-table-column>
                <vxe-table-column width="120" field="gongcheng" title="工程名称" ></vxe-table-column>
                <vxe-table-column width="120" field="createCname" title="管理员" ></vxe-table-column>
                <vxe-table-column width="120" field="steelName" title="品名" ></vxe-table-column>
                <vxe-table-column width="120" field="steelGuige" title="规格" ></vxe-table-column>
                <vxe-table-column width="120" field="steelPaihao" title="材质" ></vxe-table-column>
                <vxe-table-column width="120" field="steelPinpai" title="钢厂" ></vxe-table-column>
                <vxe-table-column width="120" field="steelWorks" title="提货方式" ></vxe-table-column>
                <vxe-table-column width="120" field="jiaohuodi" title="交货地" ></vxe-table-column>
                <vxe-table-column width="120" field="jianshu" title="件数" ></vxe-table-column>
                <vxe-table-column width="120" :formatter="this.$global.vxeTableWeight" field="weight" title="单件重" ></vxe-table-column>
                <vxe-table-column width="120" :formatter="this.$global.vxeTableWeight" field="zongzhongliang" title="总重量" ></vxe-table-column>
                <vxe-table-column width="120" :formatter="this.$global.vxeTableMoney" field="guapaijia" title="单价" ></vxe-table-column>
                <vxe-table-column width="120" :formatter="this.$global.vxeTableMoney" field="cusMoney" title="总金额" ></vxe-table-column>
                <vxe-table-column width="120" field="kunbaohao" title="捆包号" ></vxe-table-column>
                <vxe-table-column width="120"  :formatter="this.$global.vxeTableMoney" field="internetMoney" title="网价" ></vxe-table-column>
                <vxe-table-column width="120"  :formatter="this.$global.vxeTableMoney" field="floatMoney" title="浮动价格" ></vxe-table-column>
                <vxe-table-column width="120" field="region" title="网价区域" ></vxe-table-column>
                <vxe-table-column width="120" field="internetMoneyNote" title="网价备注" ></vxe-table-column>
                <vxe-table-column width="120"  :formatter="this.$global.vxeTableMoney" field="jiesuanDanjia" title="结算单价" ></vxe-table-column>
                <vxe-table-column width="120" :formatter="this.$global.vxeTableWeight" field="jiesuanWeight" title="结算重量" ></vxe-table-column>
                <vxe-table-column width="120"  :formatter="this.$global.vxeTableMoney" field="jiesuanMoney" title="结算金额" ></vxe-table-column>
                <vxe-table-column width="120" field="receiptStatus" :formatter="receiptStatus" title="是否收货" ></vxe-table-column>
                <vxe-table-column width="120" :formatter="this.$global.vxeTableWeight" field="invoiceWeight" title="开票重量" ></vxe-table-column>
                <vxe-table-column width="120"  :formatter="this.$global.vxeTableMoney" field="invoiceMoney" title="开票金额" ></vxe-table-column>
                <vxe-table-column width="120" field="invoiceNo" title="开票号码" ></vxe-table-column>
                <vxe-table-column width="120" field="invoiceStatus" :formatter="invoiceStatus" title="是否收票" ></vxe-table-column>
                <vxe-table-column width="120"  :formatter="this.$global.vxeTableMoney" field="buyRealDanjia" title="采购单价" ></vxe-table-column>
                <vxe-table-column width="120" :formatter="this.$global.vxeTableWeight" field="buyRealWeight" title="采购结算重量" ></vxe-table-column>
                <vxe-table-column width="120"  :formatter="this.$global.vxeTableMoney" field="buyRealMoney" title="采购结算金额" ></vxe-table-column>
...
import {findOrgCusAcc,getUserList,findOrdersInfoAllSum89Zong} from '@/api/data'
...
export default {
    ...
    methods:{
      ...
      getAdmin(item){
       ...
       serachBtn(){
         console.log("订单号："+this.formItem.orderNo);
         console.log("创建时间："+this.formItem.beginTime);
         console.log("截止时间："+this.formItem.endTime);
         console.log("订单状态："+this.formItem.orderStatus);
             return false;
         this.searchM();
       },
       //查询表格
