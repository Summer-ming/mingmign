 ...
 * @Last Modified time: 2019-10-28 15:22:53
 */
<!--
 * @Description: 客户余额表
 * @Author: wb
...
export default {
    ...
    data(){
      return {
        ...
        columns4: [
                    ...
                    //5-(9+10)  = 未开票款；5和 9 ；5是 应开票款;9+10是 已开票款
                    //
                     {
                        title: '客户未开票额',
                        key: 'key1',
                    ...
                    {
                        title: '已开票款（元）',//9+10是 已开票款
                        key: 'key2',
                        align: 'center',
                        // width:'200'
                        render:(h,params) =>{
                          return h('div',{
                          },this.$global.isMoneyShow(params.row.key2))
                        }
                    },
                    {
                        title: '应收票款（元）',//5是 应开票款
                        key: 'outMoney5',
                        align: 'center',
                        // width:'120',
                        render:(h,params) =>{
                          return h('div',{
                          },this.$global.isMoneyShow(params.row.outMoney5))
                        }
    ...
    methods:{
      ...
      getDian(item){
        ...
        console.log(item)
        this.formItem.orgId = item.value;
        this.formItem.orgName = item.label;
       },
      ...
      getAdmin(item){
       ...
       searchM(){
        ...
        //  param.userId      = this.adminSearchId  //管理员查询
          ...
          param.orgName = this.formItem.orgName;
           findOrgGroup(param).then(res=>{
            if(res.code =="100"){
            ...
            this.data1.map(item =>{
              //客户未开票额
                          let a = this.$global.accAdd(item.outMoney9,item.outMoney10)
                          let b = this.$global.accMinus(item.outMoney5,a)
                          item.key1 = b; 
              //    已开票款（元）
                          let c = this.$global.accAdd(item.outMoney9,item.outMoney10)
                          item.key2 = c;
            })
          }  
         })
