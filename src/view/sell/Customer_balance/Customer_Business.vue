 ...
 * @Last Modified time: 2019-11-27 15:37:04
 */
<!--
 * @Description: 客户余额表
 * @Author: wb
...
<template>
     ...
     <Form :model="formItem" inline :label-width="100">
        <Row>
          <i-Col span="24">
           <FormItem label="公司名称:">
            <Select v-model="orgSearch" id='kehu'  style="width:200px" filterable :placeholder="placeName"  @on-change='getDianOrg'  :label-in-value='true'>
         ...
         <FormItem label="本公司抬头:">
          ...
          <i-Col span="24">
             ...
             <Button style="margin-left:10px" type="primary"  icon="ios-download-outline" @click="exportData()">导出excel</Button>
             <Button style="margin-left:10px" type="primary"  @click="orderBack">订单退款客户</Button>
             <Button style="margin-left:10px" type="primary"  @click="daoBack">到账单退款客户</Button>
          </i-Col>
        </Row>
        ...
        <vxe-table height="400"
         class="radio-table"  border  size="mini" ref="xTable1" :data.sync="data1"  @cell-click="cellClickEvent"  >
                <vxe-table-column type="radio" width="60"></vxe-table-column>
                <vxe-table-column width="100" field="orgId" title="ID" ></vxe-table-column>
                <vxe-table-column  width="250" field="orgName" title="公司名称" ></vxe-table-column>
                 <vxe-table-column  width="250" field="engineeringSo" title="工程名称" ></vxe-table-column>

                <vxe-table-column width="250"   :formatter="this.$global.vxeTableMoney" field="key1" title="客户角色余额" ></vxe-table-column>
                <vxe-table-column  width="250"  :formatter="this.$global.vxeTableMoney" field="inMoney" title="到账金额(元)(入)" ></vxe-table-column>
                <vxe-table-column  width="250"  :formatter="this.$global.vxeTableMoney" field="outMoney" title="应收金额(元)(出)" ></vxe-table-column>
                <vxe-table-column width="150" title="操作" fixed="right">
                  <template v-slot="{row}">
                    <!-- <Button size="small" @click="seeDataItem(row)" type="primary" style='margin-right:10px;'>查询</Button> -->
                     <Button size="small" @click="chaKan(row)" type="primary">新查询明细</Button>
                  </template>
...
findShop,
addJgJieSuan,
updateJieSuanForStatus1,findOrderEngineering
} from '@/api/data'
...
export default {
    ...
    methods:{
      ...
      seeDataItem(row){
        sessionStorage.setItem('compay',JSON.stringify(row))
        const route = {
      ...
      orderBack(){
        ...
        // if(this.$refs.xTable1.getRadioRow() == null){
         
        //   return false;
        // }
        this.clickItem={};//清空
        this.clickItem = this.seleData;
      ...
      getDianOrg(item){
        ...
        this.formItem.orgId = jsonStr.orgid
      },
      getDian(item){
      ...
      getAdmin(item){
       ...
       searchM(){
        ...
        //  param.userId      = this.adminSearchId  //管理员查询
         param.orgId = this.formItem.orgId  //供应商
        if(this.placeName=='请选择公司名称'){
            param.orgName =  this.formItem.companyName;//公司名称
        }else{
            param.orgName=this.$route.query.customerOrgName
        }
         ...
         param.endTime     = this.formItem.endTime        
          ...
          //过滤 只查询 有效数据
           findOrgMoneyRecordMoneySum(param).then(res=>{
            ...
            this.data1.map(item=>{
                //客户角色余额 客户角色余额 = 到账金额 - 应收金额 2019年7月9日10:50:58
                let a = this.$global.accMinus(item.inMoney,item.outMoney);
                item.key1 = a;
                //
            })
          }  
    ...
    　　　　//this.loadPageData(to.query.id) "/sell/Customer_balance/org_money_record_list
           if(to.fullPath.indexOf('/sell/Customer_balance/Customer_Business?id') !=-1){
            
             this.placeName=this.$route.query.customerOrgName
             this.searchM()
...
　
    ...
    created(){//处理第一次监控不到路由
      ...
      // if(JSON.stringify(this.$route.params) == "{}"){//这是true
      //       this.placeName=this.placeName
      // }else{//这是false
      //   this.placeName=this.$route.params.customerOrgName
      // }
